...
def single_selsa_gpu_test(model, data_loader, all_frame_interval=21, show=False, rank=0, world_size=1):
    ...
    all_bboxes = [None for i in num_images]

    t = time.time()
    for i, data in enumerate(data_loader):
        batch_size = batch_size if batch_size else len(data['img_meta'].data[0])
        img_meta = data['img_meta']
        img_meta.data[0] = img_meta.data[0][0]
        frame_offset = img_meta.data[0]['frame_offset']
        key_frame_flag = img_meta.data[0]['key_frame_flag']
        seg_len = img_meta.data[0]['seg_len']
        ...
        if key_frame_flag == 0:
            ...
            if len(feat_list) < all_frame_interval - 1:
                ...
                with torch.no_grad():
                    ...
                    result = model(x=feat_list, img=None, img_meta=c_img_meta, 
                ...
                all_bboxes[frame_ids[frame_idx]-1] = results
...
def multi_selsa_gpu_test(model, data_loader, all_frame_interval, tmpdir=None, gpu_collect=False, show=False):
    ...
    results = single_selsa_gpu_test(model, data_loader, all_frame_interval,rank=rank, world_size=world_size)

    if gpu_collect:
        results = collect_results_gpu(results, size)
    else:
        results = collect_results_cpu(results, size, tmpdir)
    return results
