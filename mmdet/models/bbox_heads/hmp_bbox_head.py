...
class HMPBBoxHead(BBoxHead):
    ...
    def __init__(self, 
                 ...
                 *args,
                 **kwargs):
        super(HMPBBoxHead, self).__init__(*args, **kwargs)
    ...
    def init_weights(self):
        super(HMPBBoxHead, self).init_weights()
    ...
    def forward_single_selsa(self, 
        ...
        args:
        '''
        # assert not non_cur_space and not idx_output_cur_only, "Under HMPBBoxHead, these two options haven't been carefully reviewed"
        assert not non_cur_space, "Under HMPBBoxHead, `non_cur_space` hasn't been carefully reviewed"
        ...
        # TODO: Fix this function under HMPBBoxHead
        if non_cur_space:
    ...
    def forward_single_selsa_with_mining(self, 
        ...
        '''
        # assert not non_cur_space and not idx_output_cur_only, "Under HMPBBoxHead, these two options haven't been carefully reviewed"
        assert not non_cur_space and not idx_output_cur_only, "Under HMPBBoxHead, `non_cur_space` hasn't been carefully reviewed"
        ...
        if not test:
            ...
            if metric_loss is not None:
                ...
                m_loss = metric_loss.compute_loss(q_data_metric, k_data_metric, labels, 
                                        [ind_cls_not_bg, neg_indices, pos_indices])
        else:
    ...
    def forward(self, bbox_feat_s, cur_range_s=None, key_dim=0, all_res=False, others=None, dynamic=False, all_labels=None):
        ...
        for i, bbox_feat in enumerate(bbox_feat_s):
            ...
            del fc_all_2
            bbox_num_per_video.append(fc_all_2_relu.size(0))
        ...
        loss_metric = TripletNonLocalLoss(margin=100)
...
#TODO: Rewrite this function, most of the functionality needs to be implemented here
    def forward_test(self, bbox_feat_s, cur_range_s=None, key_dim=0, all_res=False):
        ...
        # attention_3, _ = self.forward_single_selsa_with_mining_inplace(fc_new_feat_3, None, 
        #                           self.nongt_dim, index=3, idx_output_cur_only=self.output_cur_only, test=True, k=2)
